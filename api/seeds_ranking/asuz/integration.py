...
def get_auth_token():
    auth_url = urllib.parse.urljoin(env_var('ASUZ_API_URL'), '/identity/api/Auth')
    asuz_api_login = env_var('ASUZ_API_LOGIN', '')
    asuz_api_password = env_var('ASUZ_API_PASSWORD', '')
    response = requests.post(url=auth_url, data={'login': asuz_api_login, 'password': asuz_api_password}, verify=False)
...
def get_procurements():
    ...
    resp = requests.get(url=request_url, headers=headers, verify=False)
